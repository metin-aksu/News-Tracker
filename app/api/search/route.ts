...
const BEARER = process.env.BEARER_TOKEN as string;
...
export async function GET(request: Request) {
  try {
    ...
    const userSearchUrl = `https://api.twitter.com/2/tweets/search/recent?query=${encodeURIComponent(
      qUser
    ...
    const verifiedUrl = `https://api.twitter.com/2/tweets/search/recent?query=${encodeURIComponent(
      qVerified
